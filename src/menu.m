...
function [ hand, k, path_data_file, name_data_file, level, wavelet,...
    ...
    disp('|                        Parameters                        |');
    disp(' ---------------------------------------------------------- ');
    hand = str2double(input('Rehabilitation on the\n1. right hand\n2. left hand\n','s')); 
    ...
    classifier = str2double(input(strcat('Which classifier?\n 1. kNN\n 2.', ...
        ' SVM\n 3. Multi-Layer Perceptron\n'), 's'));
    % Extra parameter for the k-NN classifier
    if isequal(classifier, 1)
       k = str2double(input('How many nearest neighbours to consider?\n', 's')); 
    end
    level = str2double(input('How many level for the DWT? (recommnded: 5)\n', 's'));
