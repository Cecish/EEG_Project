...
% Return: 
%   - final_mat_X: raw EEG data transformed into a 2D matrix from the 3D
%   matrix directly obtained with the EEGLAB toolbox
%   - ex_events_Y: target field of length nb_samples, where nb_samples
%   corresponds the number of samples (rows) of the 2D matrix final_mat_X
%   - ex_events: target field of length 40 for the 40 trials involved in
%   the experiments
function [final_mat_X, ex_events_Y, ex_events] = convertEEGLABdataIntoCsv(....
    nb_dataset, alleeg, hand)
    ...
    % Save useful information into matlab variables - Uncomment the line to
    % get these data into Matlab variables
...
%   - length_mat: number of sammples from the original 2D data obtained
%   with the EEGLAB toolbox
% Return: 
%   - ex_events_Y: target field of length nb_samples, where nb_samples
%   corresponds the number of samples (rows) of the 2D matrix final_mat_X
%   - ex_events: target field of length 40 for the 40 trials involved in
...
function ex_events = extract_events(alleeg, state, non_state)
    ...
    ex_events = zeros(1, size(alleeg.epoch, 2)); %Initialisation
